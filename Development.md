...
| 항목 | 내용 |
|---|---|
| 영어 이름 변환 | `EnglishKoreanNameService` 사전 항목 확대 |
| 정규화 보강 | 아포스트로피, 백틱, 연속 공백 정리 추가 |
| 발음 규칙 보강 | `wh`, `ck`, `ng`, `oi/oy`, `au`, `oa`, `ie` 조합 보강 |
| 관리자 API | `GET /api/admin/signatures` 추가 |
...
## 관리자 페이지 UI/UX 최종 고도화 (2026-05-14)

| 항목 | 내용 |
|---|---|
| 전체 한국어화 | 모든 관리자 페이지의 제목, 컬럼명, 버튼, 메시지를 한국어로 통일 (i18n 보강) |
| 사용자 필터링 | 관리자 사용자 관리 페이지에 ROLE(ADMIN/USER)별 필터링 기능 추가 |
| CSV 인코딩 수정 | 엑셀에서 CSV 오픈 시 한글 깨짐 방지를 위해 UTF-8 BOM(0xEF, 0xBB, 0xBF) 추가 |
| 데이터 브라우저 | 컬럼명(id, email 등)을 '번호', '이메일' 등 한국어로 자동 변환하는 헬퍼 적용 |
| 기능 정리 | 불필요한 '증명서 일괄 다운로드' 버튼 및 영문 설명 멘트 제거 |
| 백엔드 보강 | `UserRepository`에 `findByRole` 메서드 추가 및 `AdminUserController` 쿼리 파라미터 지원 |
